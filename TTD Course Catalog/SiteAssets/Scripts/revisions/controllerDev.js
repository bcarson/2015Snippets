...
/***************************************************
  ...
  **************************************************/
	...
	$scope.searchfilters.course = function(arr, option){
	...
	$scope.searchfilters.classes = function(option){
...
angular.module('filters', []) 
  ...
  .filter('courseoptions', function() {
    return function(input, options) {
    	...
    	//console.log('skill: ' + input[0].SkillLevel);
    	
    	
    	var disc = options.disc.indexOf(input[0].PrimaryDiscipline);
		var skill = options.skill.indexOf(input[0].SkillLevel);
       	if(options.disc.length === 0 && options.skill.length === 0){
    		//console.log('both arrays are empty - ' + input[0].Title);
    		return input;
    		
		} else if ( disc > -1 || skill > -1) {
				//console.log('this is in an array: ' + input[0].Title);
				return input;
		} 
			for (var i = 0; i < input[0].AdditionalDisciplines.results.length; i++) {
				...
				if (index > -1) {
					//console.log('additional discipline is in array: ' + input[0].Title);
					return input;
				}
			}
