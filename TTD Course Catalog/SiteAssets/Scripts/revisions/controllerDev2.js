			...
			if(spListData.classes.length != 0 && spListData.pics.length != 0 && spListData.success.length != 0 && spListData.courses.length != 0){
				...
				$scope.$apply(function(){
					...
					for (var i = 0; i < countSkills.length; i++) {
						$scope.SkillLevel.push(countSkills[i]);
					}
  ...
  **************************************************/
	...
	$scope.searchfilters.checkSkill = function(option){
		...
		if (skill > -1) { // if already in array
			...
			alert(option + ' already in array, removing. New array: ' + $scope.searchfilters.skill);			
		} else { // if discipline is newly selected 
			$scope.searchfilters.skill.push(option);
			alert(option + ' not in array, adding. New array: ' + $scope.searchfilters.skill);
		}	
...
angular.module('filters', []) 
  ...
  .filter('courseoptions', function() {
    return function(input, options) {
		// get input and options, assign variables, set matches false
		
		var disc = options.disc;
		var discIndex = disc.indexOf(input[0].PrimaryDiscipline);
		var discMatch = checkDisc()
		var skill = options.skill;
		var skillMatch = checkSkill()
		...
		function checkDisc() { 
			var discMatch = false;
			if ( disc.length === 0 ) { // If Discipline array is empty, return all results.
				discMatch = true;				
				console.log(input[0].Title + ' disc array empty.');
				
			} else if ( discIndex > -1 ) { // If Primary Discipline matches selection, return match.
				
				console.log(input[0].Title + ' MATCHES ' + disc);
				discMatch = true
			} else {
				console.log(input[0].Title + '  Primary does not match ' + disc)
				
				// if Primary Discipline doesn't match, check Additional Disciplines
				for ( var i = 0; i < input[0].AdditionalDisciplines.results.length; i++ ) {
					var ad = input[0].AdditionalDisciplines.results[i];
					var adIndex = disc.indexOf(ad);
					
					if ( adIndex > -1 ) {
						discMatch = true;
						console.log(input[0].Title + ' additional discipline matches');
					}
				}
			}
			return discMatch;
		...
		function checkSkill() {
			...
			if ( skill.length === 0 ) {
				skillMatch = true;
				console.log('skill array emtpy.');
			} else { 
				console.log('Skill array: ' + JSON.stringify(skill));
				for ( var i = 0; i < skill.length; i++ ) {
					if ( input[0].SkillLevel === skill[i] ) {
						skillMatch = true;
						console.log(input[0].SkillLevel + ' matches ' + skill[i] + ' match: ' + skillMatch);
					} else { 
						skillMatch = false;
						console.log(input[0].SkillLevel + ' does NOT match ' + skill[i] + ' match: ' + skillMatch); 
					}
				}	 
			}
			return skillMatch;
		...
		/*
	    var disc = discMatch();
	    var skill = skillMatch();
    	 
    	if ( disc && skill ) {
    		return input;
    	}
	 
	    function discMatch(){ // If no Disciplines are checked or if Primary Discipline matches, return match.
	    	var match = false;
	    	var discIndex = options.disc.indexOf(input[0].PrimaryDiscipline);
	    	
	    	if ( options.disc.length === 0 || discIndex > -1) {
	    		match = true;
	    		console.log(input[0].Id + ' ' + input[0].Title + ' ' + input[0].PrimaryDiscipline + ' PD matches option: ' + options.disc);
	    	} else {	    	
	    		// Else, if Additional Discipline matches, return match.
		    	for (var i = 0; i < input[0].AdditionalDisciplines.results.length; i++) {
					var ad = input[0].AdditionalDisciplines.results[i]; 
					...
					if ( adIndex > -1 ) {
						match = true;
						console.log(input[0].Id + ' ' + input[0].Title + ' ' + ad + ' AD matches option: ' + options.disc);
					}
		    	}
		    }  
	    	return match;
	    ...
	    function skillMatch(){
	    	var match = false;
			var checkedSkills = options.skill;
			var thisSkill = setSkills(input[0].SkillLevel);
			// If no Skills are checked or if Skill Level matches, return match.
			if ( checkedSkills.length === 0 ) {
				match = true;
			} else {
				for ( var i = 0; i < checkedSkills.length; i++ ) {
					if (checkedSkills[i].order === thisSkill[0]) {
						match = true;
					} 
				}
			}
